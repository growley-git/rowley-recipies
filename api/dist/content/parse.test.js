...
describe("parseRecipeFileSource", () => {
    it("parses fixture minimal.md", async () => {
        const raw = await readFile(path.join(here, "__fixtures__", "minimal.md"), "utf8");
    ...
    it("throws when no direction steps", async () => {
        const raw = await readFile(path.join(here, "__fixtures__", "invalid-no-steps.md"), "utf8");
        expect(() => parseRecipeFileSource(raw, "bad.md")).toThrow(ContentParseError);
    });
});
