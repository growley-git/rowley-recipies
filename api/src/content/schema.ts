...
export function validateFrontmatter(
  raw: RecipeFrontmatterRaw,
  filePath?: string
): ValidatedRecipeFrontmatter | ContentParseError {
  if (!raw.title?.trim()) {
  ...
  if (raw.publishedAt?.trim()) {
    ...
    if (Number.isNaN(d.getTime())) {
      return new ContentParseError(
        `Invalid publishedAt: ${raw.publishedAt}`,
        filePath
      );
    }
    publishedAt = d;
  ...
  const ingredients: RecipeIngredientYaml[] = [];
  for (let i = 0; i < ingredientsIn.length; i++) {
    const ing = ingredientsIn[i];
    if (!isRecord(ing)) {
      return new ContentParseError(
        `ingredients[${i}] must be an object`,
        filePath
      );
    }
    ...
    if (typeof foodName !== "string" || !foodName.trim()) {
      return new ContentParseError(
        `ingredients[${i}].foodName required`,
        filePath
      );
    }
    ...
    if (typeof amount !== "number" || !Number.isFinite(amount)) {
      return new ContentParseError(
        `ingredients[${i}].amount must be a number`,
        filePath
      );
    }
    ingredients.push({
