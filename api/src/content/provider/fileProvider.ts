...
import type { ParsedRecipeFile } from "../parse.js";
...
function toGraphQLNode(p: ParsedRecipeFile): RecipeGraphQLNode {
...
function matchesFilter(node: RecipeGraphQLNode, filter: RecipeFilter): boolean {
  ...
  if (filter.search?.trim()) {
    ...
    const inIng = node.ingredients.some((i) =>
      i.foodName.toLowerCase().includes(q)
    );
    const inSteps = node.steps.some((s) => s.text.toLowerCase().includes(q));
    if (!inTitle && !inIng && !inSteps) return false;
  }
  return true;
}

export class FileRecipeContentProvider implements RecipeContentProvider {
  readonly source = "files" as const;

  private cache: RecipeGraphQLNode[] | null = null;

  constructor(private readonly recipesDir: string = getContentRecipesDir()) {}

  supportsRecipeMutations(): boolean {
    return false;
  }

  private async loadAll(): Promise<RecipeGraphQLNode[]> {
    if (this.cache && !contentHotReload()) {
      return this.cache;
    }
    const paths = await listRecipeMarkdownFiles(this.recipesDir);
    const nodes: RecipeGraphQLNode[] = [];
